...
# Cache for backtest comparison
_comparison_cache = None
_comparison_cache_time = None


def run_backtest_comparison() -> dict:
    """
    Run comparison between standard and EMA-smoothed backtest strategies.
    Results are cached for the same duration as regular backtest.
    """
    global _comparison_cache, _comparison_cache_time

    # Check cache
    if _comparison_cache is not None and _comparison_cache_time is not None:
        cache_age = datetime.now() - _comparison_cache_time
        if cache_age < timedelta(hours=CACHE_DURATION_HOURS):
            return _comparison_cache

    try:
        print("=" * 60, flush=True)
        print("RUNNING BACKTEST COMPARISON (Standard vs EMA-Smoothed)", flush=True)
        print("=" * 60, flush=True)

        from quad_portfolio_backtest import QuadrantPortfolioBacktest
        from test_ema_smoothed_backtest import EMASmoothedQuadBacktest
        import numpy as np
        ...
        print("\n[1/2] Running STANDARD backtest...", flush=True)
        ...
        # Count regime changes in standard
        standard_regime_changes = 0
        if hasattr(standard_backtest, 'quad_history') and standard_backtest.quad_history is not None:
            prev_top2 = None
            for idx in standard_backtest.quad_history.index:
                current_top2 = (
                    standard_backtest.quad_history.loc[idx, 'Top1'],
                    standard_backtest.quad_history.loc[idx, 'Top2']
                )
                if prev_top2 is not None and current_top2 != prev_top2:
                    standard_regime_changes += 1
                prev_top2 = current_top2

        # Run EMA-smoothed backtest
        print("\n[2/2] Running EMA-SMOOTHED backtest...", flush=True)
        ...
        smoothed_regime_changes = getattr(smoothed_backtest, 'regime_changes', 0)

        # Build equity curves for charting
        standard_curve = []
        smoothed_curve = []

        if standard_backtest.portfolio_value is not None:
            for date, value in standard_backtest.portfolio_value.items():
                standard_curve.append({
                    'date': date.strftime('%Y-%m-%d'),
                    'value': float(value)
                })

        if smoothed_backtest.portfolio_value is not None:
            for date, value in smoothed_backtest.portfolio_value.items():
        ...
        _comparison_cache = {
            'standard': {
                'name': 'Standard (No Smoothing)',
                'total_return': standard_results.get('total_return', 0),
                'annual_return': standard_results.get('annual_return', 0),
                'sharpe': standard_results.get('sharpe', 0),
                'max_drawdown': standard_results.get('max_drawdown', 0),
                'volatility': standard_results.get('annual_vol', 0),
                'final_value': standard_results.get('final_value', INITIAL_CAPITAL),
                'regime_changes': standard_regime_changes,
                'equity_curve': standard_curve,
            },
            'smoothed': {
                'name': f'EMA-Smoothed ({EMA_SMOOTHING_PERIOD}-period)',
                'total_return': smoothed_results.get('total_return', 0),
                'annual_return': smoothed_results.get('annual_return', 0),
                'sharpe': smoothed_results.get('sharpe', 0),
                'max_drawdown': smoothed_results.get('max_drawdown', 0),
                'volatility': smoothed_results.get('annual_vol', 0),
                'final_value': smoothed_results.get('final_value', INITIAL_CAPITAL),
                'regime_changes': smoothed_regime_changes,
                'equity_curve': smoothed_curve,
            },
            'parameters': {
                'initial_capital': INITIAL_CAPITAL,
            ...
            'verdict': {
                'better_sharpe': 'smoothed' if smoothed_results.get('sharpe', 0) > standard_results.get('sharpe', 0) else 'standard',
                'better_return': 'smoothed' if smoothed_results.get('total_return', 0) > standard_results.get('total_return', 0) else 'standard',
                'fewer_regime_changes': 'smoothed' if smoothed_regime_changes < standard_regime_changes else 'standard',
            },
            'generated_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
        }
        _comparison_cache_time = datetime.now()

        print(f"\n✓ Comparison complete!", flush=True)
        print(f"  Standard: {standard_results.get('total_return', 0):.1f}% return, Sharpe {standard_results.get('sharpe', 0):.2f}", flush=True)
        print(f"  Smoothed: {smoothed_results.get('total_return', 0):.1f}% return, Sharpe {smoothed_results.get('sharpe', 0):.2f}", flush=True)

        return _comparison_cache

    except Exception as e:
        print(f"Error running backtest comparison: {e}", flush=True)
        import traceback
        traceback.print_exc()

        # Return empty comparison on error
        return {
            'standard': {'name': 'Standard', 'error': str(e)},
            'smoothed': {'name': 'EMA-Smoothed', 'error': str(e)},
            'parameters': {},
            'verdict': {},
            'error': str(e),
            'generated_at': datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
